...
$user = 'an1rakanbot'.$message->from->username;
...
if (preg_match("/\/bc .*/", $text) and $chat_id == 175279237) {
    ...
    $info = json_decode(file_get_contents("https://api.telegram.org/bot".API_KEY."/getChatMember?chat_id=@an2rakan&user_id=".$chat_id));
$per = $info->result->status;
if ($per == 'left') {
  bot('sendMessage',[
    'chat_id'=>$chat_id,
    'text'=>" عليك الأشتراك في القناه لأستخدام اشترك في القناه @an2rakan"
  ]);
}
...
bot( sendaudio ,[
 chat_id =>$chat_id, 
 audio =>"https://t.me/ANGAM_iq/4337",
 reply_to_message_id =>$message->message_id, 
]);
}
